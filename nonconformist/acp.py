...
# Authors: Henrik Linusson, Michael Gostler
...
class CrossSampler(object):
    ...
    def gen_samples(self, y, n_samples, problem_type):
        if problem_type == 'classification':
            folds = StratifiedKFold(n_splits=n_samples, test_size=self.cal_portion)
        else:
            folds = KFold(n_splits=n_samples)
        for train, cal in folds.split(np.zeros(y.shape[0]), y):
            yield train, cal
...
class RandomSubSampler(object):
    ...
    def gen_samples(self, y, n_samples, problem_type):
        if problem_type == 'classification':
            splits = StratifiedShuffleSplit(n_splits=n_samples, test_size=self.cal_portion)
        else:
            splits = ShuffleSplit(n_splits=n_samples, test_size=self.cal_portion)

        for train, cal in splits.split(np.zeros(y.shape[0]), y):
            yield train, cal
...
class CrossConformalClassifier(AggregatedCp):
    ...
    def __init__(self, predictor, n_models=10):
        super(CrossConformalClassifier, self).__init__(predictor, CrossSampler(), n_models)

    def predict(self, x, significance=None):
        predictions = np.dstack([p.predict(x, significance=None) for p in self.predictors])
        n_cal_tot = 0
...
class BootstrapConformalClassifier(AggregatedCp):
    ...
    """
    def __init__(self, predictor, n_models=10):
        super(BootstrapConformalClassifier, self).__init__(predictor, BootstrapSampler(), n_models)
